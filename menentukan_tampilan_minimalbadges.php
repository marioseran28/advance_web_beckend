...
<div class="box-body">
            ...
            <div class="btn-group dropend">
                 ...
                 $count_id=$this->db->query("SELECT COUNT(id_peminjaman) AS jumlah_total FROM tb_peminjaman_buku WHERE keterangan='dipinjam' 
                    ...
                    if ($brp_id->jumlah_total>9) {
                         echo "<button title='Notifikasi' onclick='hilang_badges()' class='btn btn-link btn-xs' style='color: black;' data-bs-toggle='dropdown'><i class='fa fa-bell-o fa-lg'><input onclick='hilang_badges()' id='button_notif' style='background-color: red; width: 25px; height: 25px; border-radius: 100%; text-align: center; border-color: red; font: 8px;' value='9+' readonly /></i></button>";

                    } else if ($brp_id->jumlah_total==9) {
                       echo "
                          <button title='Notifikasi' onclick='hilang_badges()' class='btn btn-link btn-xs' style='color: black;' data-bs-toggle='dropdown'><i class='fa fa-bell-o fa-lg'><input onclick='hilang_badges()' id='button_notif' style='background-color: red; width: 25px; height: 25px; border-radius: 100%; text-align: center; border-color: red; font: 8px;' value='$brp_id->jumlah_total' readonly />
                              </i></button>"; 
